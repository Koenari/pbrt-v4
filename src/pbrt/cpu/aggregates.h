...
public:
    ...
    virtual Bounds3f Bounds() const = 0;
    virtual pstd::optional<ShapeIntersection> Intersect(const Ray& ray, Float tMax) const
    = 0;
    virtual bool IntersectP(const Ray& ray, Float tMax) const = 0;
    bool metricsEnabled;

protected:
    BVHAggregate(int maxPrimsInNode, std::vector<Primitive> p,
    ...
    SplitMethod splitMethod;
    Float epoRatio;
    Float penalizedHitProbability(int idx, int count, ICostCalcable** buckets,
                                  const Bounds3f& parent) const;
    Float childCost(int idx, int count, ICostCalcable** buckets,
                    const Bounds3f& parent) const;
    Float splitCost(int count, ICostCalcable** buckets, const Bounds3f& parent) const;
...
protected:
    WideBVHAggregate(int TreeWidth, int maxPrimsInNode, std::vector<Primitive> p,
                     SplitMethod splitMethod,
                     Float epoRatio);
    ...
    Float calcMetrics(const WideBVHBuildNode* root, int depth);
    Bounds3f bounds;
};
...
public:
    ...
    Intersect(const Ray& ray, Float tMax) const override;
...
private:
    ...
    FourWideBVHBuildNode* buildRecursive(ThreadLocal<Allocator>& threadAllocators,
                                         ...
                                         int splitVariant);
    int flattenBVH(const FourWideBVHBuildNode* node, int* offset);
    FourWideBVHBuildNode* collapseBinBVH(const BVHBuildNode* root,
                                         std::atomic<int>* totalNodes,
                                         int collapseVariant);
...
protected:
    int8_t relevantAxisIdx(const int a1, const int a2) const override {
...
private:
    ...
    EightWideBVHBuildNode* buildRecursive(ThreadLocal<Allocator>& threadAllocators,
                                          ...
                                          std::vector<Primitive>& orderedPrims,
                                          int splitVariant);
    EightWideBVHBuildNode* collapseBinBVH(const BVHBuildNode* root,
                                          std::atomic<int>* totalNodes,
                                          int collapseVariant);
    int flattenBVH(const EightWideBVHBuildNode* node, int* offset);
    static int traversalIdx(const int dirIsNeg[3], const SIMDEightWideLinearBVHNode* node,
                            int idx);
    // FourWideBVHAggregate Private Members
...
private:
    ...
    SixteenWideBVHBuildNode* buildRecursive(ThreadLocal<Allocator>& threadAllocators,
                                            ...
                                            std::vector<Primitive>& orderedPrims,
                                            int splitVariant);
    SixteenWideBVHBuildNode* collapseBinBVH(const BVHBuildNode* root,
                                            std::atomic<int>* totalNodes,
                                            int collapseVariant);
    int flattenBVH(const SixteenWideBVHBuildNode* node, int* offset);
    static int traversalIdx(const int dirIsNeg[3],
...
private:
    ...
    Float splitCost(BVHSplitBucket left, BVHSplitBucket right,
